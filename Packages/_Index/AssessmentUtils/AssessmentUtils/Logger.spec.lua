return function()
    ...
    describe("with positional arguments", function()
        ...
        it("should output a warning with too many arguments", function()
            ...
            assert(
                string.find(sink.seen[1].message, "LUMBERYAK INTERNAL"),
        ...
        it("should ignore a nil named argument", function()
            local log = Logger.new()
            local sink = newSink(Logger.Levels.Info)
            log:addSink(sink)
    ...
    it("Should get the name of logger used", function()
        ...
        expect(sink.seen[2].message).to.equal("log1.log2")
    end)
    ...
    describe("a logging tree", function()
        it("should support building log tree", function()
            Logger.init(true)
            local root = Logger.get()
            local grandchild = Logger.get("child.grandchild")
            local child = root:getChild("child")
        ...
        it("should output the log hierarchy", function()
            Logger.init(true)
            local root = Logger.get()
            local grandchild = Logger.get("child.grandchild")
            ...
            grandchild:info("Log Message")

            expect(sink.seen[1].context.name).to.equal("child.grandchild")
        end)
    end)
end
